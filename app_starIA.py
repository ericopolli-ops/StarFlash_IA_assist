...
# ==========================================
# ABA 4: ASSISTENTE COMERCIAL IA (VOZ/CHAT)
# ==========================================
with aba4:
    st.markdown("### 🤖 Assistente Comercial Inteligente (IA & Voz)")
    st.markdown("Converse com os dados da Star Flash. O assistente entende o contexto comercial, extrai os filtros e cruza com a base em tempo real.")
    st.markdown("💡 *Exemplos de comandos:* `pedidos abertos da pirelli`, `faturamento pirelli em junho 2026`, `pedidos da transportadora`")

    # Histórico de conversas da IA na sessão
    if "mensagens_chat_ia" not in st.session_state:
        st.session_state.mensagens_chat_ia = [
            {"role": "assistant", "content": "Fala chefe! O que você quer puxar da base da Star Flash por comando de voz/texto agora?"}
        ]
    ...
    # Entrada de comando em texto (simulando a fala ou digitação rápida do celular)
    if prompt_ia := st.chat_input("Digite o comando (ex: pedidos faturados pirelli junho 2026)...", key="chat_input_ia"):
        st.session_state.mensagens_chat_ia.append({"role": "user", "content": prompt_ia})
        with st.chat_message("user"):
            st.markdown(prompt_ia)

        with st.chat_message("assistant"):
            with st.spinner("Interpretando comandos e consultando o E2DW..."):
                try:
                    texto_lower = prompt_ia.lower()
                    
                    # Query flexível inteligente
                    query_assistente = "SELECT PedVenda, Status, Nome_Clien, Cidade, Dt_Fatura, Data_Ped, i_Vtotal, i_NomeProd, TRANSPORTA FROM PEDIDODEVENDA WHERE 1=1"
                    params_assistente = {}

                    # Segurança por perfil de vendedor
                    if st.session_state.perfil == "VENDEDOR":
                        query_assistente += " AND Vendedor LIKE %(v_log)s"
                        params_assistente["v_log"] = f"%{st.session_state.vendedor_nome}%"

                    # Filtro inteligente de Cliente (Pirelli, Dana, etc.)
                    if "pirelli" in texto_lower:
                        query_assistente += " AND Nome_Clien LIKE '%PIRELLI%'"
                    elif "dana" in texto_lower:
                        query_assistente += " AND Nome_Clien LIKE '%DANA%'"

                    # Filtro inteligente de Status (Aberto vs Faturado)
                    ...
                    # Filtro inteligente de Período/Mês (Ex: Junho 2026 -> 202606)
                    if "junho" in "junho 2026" or "2026" in texto_lower:
                        if "junho" in texto_lower:
                            query_assistente += " AND (Dt_Fatura LIKE '202606%%' OR Data_Ped LIKE '202606%%')"
                    
                    query_assistente += " LIMIT 250;"
                    df_res_ia = pd.read_sql(query_assistente, engine, params=params_assistente)

                    if df_res_ia.empty:
                        resposta_ia = "Não encontrei nenhum registro correspondente a esse comando na base de dados."
                        st.markdown(resposta_ia)
                    else:
                        total_valor = df_res_ia['i_Vtotal'].sum()
                        total_pedidos = df_res_ia['PedVenda'].nunique()
                        
                        resposta_ia = f"Encontrei **{total_pedidos} pedido(s)** atrelados, totalizando **R$ {total_valor:,.2f}**."
                        st.markdown(resposta_ia)

                        # Tabela enxuta focada na visualização mobile
                        df_show_ia = df_res_ia[['PedVenda', 'Status', 'Nome_Clien', 'Data_Ped', 'i_Vtotal']].drop_duplicates()
                        df_show_ia.rename(columns={'PedVenda': 'Nº Pedido', 'Status': 'Status', 'Nome_Clien': 'Cliente', 'Data_Ped': 'Emissão', 'i_Vtotal': 'Valor Total'}, inplace=True)
                    ...
                    erro_str = f"❌ Erro ao processar o comando inteligente: {e}"
                    st.error(erro_str)
